...
const App = () => {
  ...
  const getPersonBadgeStyle = (personName) => {
    switch (personName) {
      ...
      case '姜佩均': return 'bg-orange-50 text-orange-700';
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl">
              ...
              <button onClick={() => setViewMode('姜佩均')} className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all ${viewMode === '姜佩均' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <User size={16} /> 姜佩均
              </button>
            </div>
          </div>
